(function () {
  ...
  $('ne-start').addEventListener('click', () => {
    ...
    const greet = `Hi ${neLead.name}! 👋 Great to connect. I'm the Nachi Eng AI assistant — here to help you understand how a **CMMS** can transform maintenance at **${neLead.company}**.\n\nYou're in the **${neSector}** sector — a key area we work in. What would you like to know?`;
    neAddBot(greet);
    neHistory = [
  ...
  async function neSend() {
    ...
    const sys = `You are the AI assistant for Nachi Eng Ltd, a UK-based Maintenance Management Systems consultancy.
...
STYLE: Conversational, helpful, never salesy. Max 4 sentences unless detail needed. Use **bold** for key terms. Always guide toward booking a call.`;
  ...
  function neAddBot(t) {
    ...
    d.innerHTML = t.replace(/\*\*(.*?)\*\*/g,'<strong>$1</strong>').replace(/\n/g,'<br>');
    $('ne-msgs').appendChild(d); neScroll();
  }
